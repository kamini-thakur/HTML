...
if (!class_exists('Dynamic_Featured_Image')) {
	class Dynamic_Featured_Image {
		...
		public function enqueue_admin_scripts() {
			...
			wp_localize_script(
				...
				array(
					...
					'metabox_title'            => __( $this->_metabox_title, 'mtlisitings' ),
					'mediaSelector_title'      => __( 'Dynamic Featured Image - Media Selector', 'mtlisitings' ),
					'mediaSelector_buttonText' => __( 'Set Featured Image', 'mtlisitings' )
				)
			);
		...
		public function initialize_featured_box() {
			...
			$this->_metabox_title = apply_filters( 'dfi_set_metabox_title', __( "Featured Image", 'mtlisitings' ) );
		...
		protected function _get_number_translation( $number ) {
			if ( $number <= 9 ) {
				return __( $number, 'mtlisitings' );
			} else {
				$pieces = str_split( $number, 1 );
				$buffer = '';
				foreach ( $pieces as $piece ) {
					$buffer .= __( $piece, 'mtlisitings' );
				}
			...
			if ( ! is_null( $i ) ) {
				foreach ( $postTypes as $type ) {
					add_meta_box(
						...
						__( $this->_metabox_title, 'mtlisitings' ) . " " . self::_get_number_translation( $i ),
				...
				foreach ( $postTypes as $type ) {
					add_meta_box(
						'dfiFeaturedMetaBox',
						__( $this->_metabox_title, 'mtlisitings' ) . " " . __( 2, 'mtlisitings' ),
						array( $this, 'featured_meta_box' ),
						$type,
						'side',
						'low',
		...
		private function _get_featured_box( $featuredImgTrimmed, $featuredImg, $featuredId, $thumbnail, $postId ) {
			...
			return "<a href='javascript:void(0)' class='dfiFeaturedImage {$hasFeaturedImage}' title='" . __( 'Set Featured Image', 'mtlisitings' ) . "' data-post-id='" . $postId . "'><span class='dashicons dashicons-camera'></span></a><br/>
				<img src='" . $thumbnail . "' class='dfiImg {$dfiEmpty}'/>
				<div class='dfiLinks'>
					<a href='javascript:void(0)' data-id='{$featuredId}' data-id-local='" . $this->_get_number_translation( ( $featuredId + 1 ) ) . "' class='dfiAddNew dashicons dashicons-plus' title='" . __( 'Add New', 'mtlisitings' ) . "'></a>
					<a href='javascript:void(0)' class='dfiRemove dashicons dashicons-minus' title='" . __( 'Remove', 'mtlisitings' ) . "'></a>
				</div>
		...
		public function ajax_callback() {
			...
			<a href="javascript:void(0)" class="dfiFeaturedImage"
			   title="<?php echo __( 'Set Featured Image', 'mtlisitings' ) ?>"><span
			...
			<div class="dfiLinks">
				<a href="javascript:void(0)" data-id="<?php echo $featuredId ?>"
				   ...
				   class="dfiAddNew dashicons dashicons-plus" title="<?php echo __( 'Add New', 'mtlisitings' ) ?>"></a>
				<a href="javascript:void(0)" class="dfiRemove dashicons dashicons-minus"
				   title="<?php echo __( 'Remove', 'mtlisitings' ) ?>"></a>
			</div>
			<div class="dfiClearFloat"></div>
		...
		public function update_notice() {
			$info = __( 'ATTENTION! Please read the <a href="https://github.com/ankitpokhrel/Dynamic-Featured-Image/wiki" target="_blank">DOCUMENTATION</a> properly before update.', 'mtlisitings' );
